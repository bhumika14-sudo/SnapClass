...
def teacher_dashboard():
    ...
    with c2:
        ...
        if st.button("Go back", type='secondary', key='loginbackbtn', use_container_width=True):
...
def teacher_tab_take_attendance():
    ...
    with col2:
        if st.button('Add Photos', type='primary', icon=':material/photo_auto_merge:', use_container_width=True):
            add_photos_dialog()
    ...
    if subjects:
        for sub in subjects:
            ...
            def share_btn(sub=sub):
                b1, b2 = st.columns(2)
                with b1:
                    # fix: unique key using both subject_code + subject_id
                    if st.button(
                        "Share Code",
                        key=f"share_{sub['subject_code']}_{sub['subject_id']}",
                        icon=":material/share:",
                        use_container_width=True
                    ):
                        share_subject_dialog(sub['name'], sub['subject_code'])
                with b2:
                    # new: remove subject button
                    if st.button(
                        "Remove",
                        key=f"remove_{sub['subject_id']}",
                        icon=":material/delete:",
                        type="secondary",
                        use_container_width=True
                    ):
                        supabase.table('subjects').delete().eq('subject_id', sub['subject_id']).execute()
                        st.toast(f"'{sub['name']}' removed successfully")
                        st.rerun()
...
def teacher_screen_register():
    ...
    with c2:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("Go back", type='secondary', key='registerbackbtn', use_container_width=True):
            st.session_state['login_type'] = None
            st.rerun()
    ...
    with btnc1:
        if st.button('Register Now', icon=':material/passkey:', use_container_width=True):
            ...
            if success:
                ...
                st.error(message)
    with btnc2:
        if st.button('Login Instead', type="primary", icon=':material/login:', use_container_width=True):
            st.session_state.teacher_login_type = 'login'
            st.rerun()